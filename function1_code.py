...
def lambda_handler(event, context):
    ...
    for _ec2_instance_id in _ec2_instances:
        try:
            print("Ligando instância:", _ec2_instance_id)
            _res = execute_ec2_start(_ec2_instance_id, region)
            print("Instância ligada:", _ec2_instance_id)
            _res_alarm = enable_alarm([_ec2_instance_id], region)
            print("Alarme habilitado:", _ec2_instance_id)
        ...
        for instance in reservation['Instances']:
            if instance['State']['Name'] == 'stopped':
                ...
                if auto_power_seg_tag and check_same_day( current_time,0):
                    ...
                    if check_time_range(auto_power_seg_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue
                
                if auto_power_ter_tag and check_same_day(current_time,1):
                   
                    if check_time_range(auto_power_ter_tag, current_time):
                ...
                if auto_power_qua_tag and check_same_day( current_time,2):
                    ...
                    if  check_time_range(auto_power_qua_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue

                if auto_power_qui_tag and check_same_day( current_time,3):
                    
                    if  check_time_range(auto_power_qui_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue

                if auto_power_sex_tag and check_same_day( current_time,4):
                    
                    if  check_time_range(auto_power_sex_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue

                if auto_power_sab_tag and check_same_day( current_time,5):
                    
                    if  check_time_range(auto_power_sab_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    break

                if auto_power_dom_tag and check_same_day( current_time,6):
                   
                    if check_time_range(auto_power_dom_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue
                #seg-sex   e    #sab-dom
                if auto_power_seg_sex_tag and check_same_day_seg_sex(current_time):
                
                    if check_time_range(auto_power_seg_sex_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue
                
                if auto_power_sab_dom_tag and check_same_day_sab_dom(current_time):
              
                    if  check_time_range(auto_power_sab_dom_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue
                #semanal
                if auto_power_semana_tag:
              
                    if  check_time_range(auto_power_semana_tag, current_time):
                        instance_ids.append(instance['InstanceId'])
                    continue
...
def ec2_instance_start(_ec2_instance_id: str, _region: str):
    """
    Start an instance
    ...
    try:
        ...
        _ec2_resource.Instance(_ec2_instance_id).start()
    except Exception as ex:
        raise Exception(ex)
    return 0


def execute_ec2_start(_ec2_instance_id: str, _region: str):
    # Try to start the instance
    try:
        _res = ec2_instance_start(_ec2_instance_id, _region)
    except Exception as ex:
        _time = datetime_to_str_pt_br(datetime.datetime.now())
        _err = '%s: Failed to start instance %s, error: %s' % (_time, _ec2_instance_id, ex.args)
        print(_err)
    ...
    try:
        if _res == 0:
            ...
            _msg = 'Instance %s (%s) has been started, launch time: %s' % (_instance_name,
                                                                           _ec2_instance_id,
                                                                           _instance_launch_time)
        else:
            _msg = 'Instance %s has not been started' % _ec2_instance_id
    except Exception as ex:
        _time = datetime_to_str_pt_br(datetime.datetime.now())
...
def datetime_to_str_pt_br(_datetime: datetime):
    """
...
def enable_alarm(_instance_ids: list, _region: str):
    try:
        _instance_alarms = filter_alarm_by_instance(_instance_ids, _region)
        enable_alarm_actions(_instance_alarms, _region)
        _time = datetime.datetime.now()
        _msg = '%s: Alarms from instance %s enabled' % (_time, _instance_ids)
    except Exception as ex:
        _time = datetime.datetime.now()
...
def enable_alarm_actions(_instance_alarms: list, _region: str = None, _profile: str = None):
    _cloudwatch_client = boto3.Session(region_name=_region).client('cloudwatch')
    for _instance_alarm in _instance_alarms:
        _cloudwatch_client.enable_alarm_actions(AlarmNames=_instance_alarm['alarms'])
